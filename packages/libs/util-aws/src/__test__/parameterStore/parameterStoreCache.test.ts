...
import { ParameterStoreCache } from '../../parameterStore/parameterStoreCache';
import { ParameterStore } from '../../parameterStore/parameterStore';
...
describe('ParameterStoreCache', () => {
  ...
  describe('getParameter', () => {
    describe('When the cache is empty', () => {
      ...
      test('should set param to cache if retrieved param is undefined', async () => {
        mockGetParamFromSourceSpy.mockResolvedValue(undefined);
  ...
  describe('addParameter', () => {
    test('should add the param to the source', async () => {
      ...
      expect(mockAddParamToSourceSpy).toBeCalledTimes(1);
      expect(mockAddParamToSourceSpy).toBeCalledWith(
        firstMyParamName,
        firstMyParam.Value!,
        ParameterType.STRING,
        true
  ...
  describe('clearCachedParameter', () => {
    test('clears SSM parameter from cache if found and no version specified', async () => {
      ...
      expect(mockDelete).toBeCalledWith('name:ssm-param');
    });
  ...
  describe('deleteParameter', () => {
    test('deletes the parameter from the source', async () => {
      ...
      expect(mockDeleteParamFromSourceSpy).toBeCalledTimes(1);
      expect(mockDeleteParamFromSourceSpy).toBeCalledWith('ssm-param');
    });
